...
**⚠️ HeyGen 口白 = 中文為主！**
- 口白內容跟語音稿（Part 1）一樣，**中文為主**
- 只有名言原文才用英文（如 "Do what you can, with what you have, where you are."）
- `--language "en"` 是 HeyGen TTS 引擎參數（因為 clone voice 訓練語言是英文），**不是要把口白改成英文！**
- 傳給 HeyGen 的文稿請移除情緒標籤（如 `[short pause]`）

**⚠️ 重要語音參數（2026-02-28 A/B 測試確認最佳設定）：**
...
### Part 3b：ZapCap 字幕（HeyGen 影片加字幕）
